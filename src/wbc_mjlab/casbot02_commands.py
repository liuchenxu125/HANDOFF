...
class Casbot02VelocityCommand(UniformVelocityCommand):
  ...
  The configured standing, pure-turn, forward, startup and heading fractions are
  ...
  def _update_command(self) -> None:
    # The walk speed is sampled when standing starts. The shared timer only
    # switches from a zero command to that stored launch speed.
    ...
    self.vel_command_w[startup] = self.vel_command_b[startup]
  ...
  def _resample_command(self, env_ids: torch.Tensor) -> None:
    ...
    # Hold zero command, then step to a forward/backward command sampled
    # when the stand phase begins. Respect the live curriculum range.
...
class Casbot02VelocityCommandCfg(UniformVelocityCommandCfg):
  ...
  def __post_init__(self) -> None:
    ...
    if self.rel_startup_envs > 0.0 and not (
      self.ranges.lin_vel_x[0] < 0.0 < self.ranges.lin_vel_x[1]
    ):
      raise ValueError("Stand-then-walk requires lin_vel_x to span both signs")
